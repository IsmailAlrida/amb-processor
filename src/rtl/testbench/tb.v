...
module cpu_tb;
  ...
  //! Debug alias for the CPU halt signal.
  wire debug_halt;
  //! Debug alias for the CPU instruction counter.
  wire [INSTRUCTION_ADDRESS_SPACE-1:0] debug_IC;
  //! Debug alias for the current instruction word.
  wire [INSTRUCTION_WIDTH-1:0] debug_instr;
  //! Debug alias for the decoded opcode.
  wire [6:0] debug_opcode;
  //! Debug alias for the CPU data memory address.
  wire [DATA_ADDR_SPACE-1:0] debug_DataAddress;
  //! Debug alias for the CPU data memory write bus.
  wire [DATA_WORD_WIDTH-1:0] debug_DataMemoryWrite;
  //! Debug alias for the CPU data memory read bus.
  wire [DATA_WORD_WIDTH-1:0] debug_DataMemoryRead;
  //! Debug alias for the data memory read enable.
  wire debug_DmemReadEn;
  //! Debug alias for the data memory write enable.
  wire debug_DmemWriteEn;
  //! Debug alias for the register file write enable.
  wire debug_RegWrite;
  //! Debug alias for the register writeback source selector.
  wire [1:0] debug_RegDest;
  //! Debug alias for memory-operation operand selection.
  wire debug_MemOp;
  //! Debug alias for equality branch control.
  wire debug_BranchEqual;
  //! Debug alias for less-than branch control.
  wire debug_BranchLt;
  //! Debug alias for jump opcode detection.
  wire debug_isJump;
  //! Debug alias for long-jump offset selection.
  wire debug_LongJump;
  //! Debug alias for immediate-load detection.
  wire debug_isImmLoad;
  //! Debug alias for the ALU operation selector.
  wire [3:0] debug_ALUCtrl;
  //! Debug alias for the IMR byte-lane selector.
  wire [1:0] debug_ImmSel;
  //! Debug alias for packed compare/jump control bits.
  wire [2:0] debug_CMPType;
  //! Debug alias for the Ra read selector.
  wire [3:0] debug_ReadRegRa;
  //! Debug alias for the Rb read selector.
  wire [3:0] debug_ReadRegRb;
  //! Debug alias for the register file write selector.
  wire [3:0] debug_WriteReg;
  //! Debug alias for Ra read data.
  wire [DATA_WORD_WIDTH-1:0] debug_ReadDataRa;
  //! Debug alias for Rb read data.
  wire [DATA_WORD_WIDTH-1:0] debug_ReadDataRb;
  //! Debug alias for ALU operand A.
  wire [DATA_WORD_WIDTH-1:0] debug_OperandA;
  //! Debug alias for ALU operand B.
  wire [DATA_WORD_WIDTH-1:0] debug_OperandB;
  //! Debug alias for the ALU result.
  wire [DATA_WORD_WIDTH-1:0] debug_ALURes;
  //! Debug alias for the ALU equality flag.
  wire debug_zero;
  //! Debug alias for the ALU less-than flag.
  wire debug_altb;
  //! Debug alias for register writeback data.
  wire [DATA_WORD_WIDTH-1:0] debug_RegDataWrite;
  //! Debug alias for the branch decision.
  wire debug_WillBranch;
  //! Debug alias for the next IC candidate.
  ...
  //! Mirrors CPU halt at testbench scope for waveform viewers.
  assign debug_halt = halt;
  //! Mirrors CPU IC at testbench scope for waveform viewers.
  assign debug_IC = IC;
  //! Mirrors the current instruction at testbench scope.
  assign debug_instr = instr;
  //! Mirrors decoded opcode from inside the CPU.
  assign debug_opcode = cpu_inst.opcode;
  //! Mirrors data memory address at testbench scope.
  assign debug_DataAddress = DataAddress;
  //! Mirrors data memory write data at testbench scope.
  assign debug_DataMemoryWrite = DataMemoryWrite;
  //! Mirrors data memory read data at testbench scope.
  assign debug_DataMemoryRead = DataMemoryRead;
  //! Mirrors data memory read enable at testbench scope.
  assign debug_DmemReadEn = DmemReadEn;
  //! Mirrors data memory write enable at testbench scope.
  assign debug_DmemWriteEn = DmemWriteEn;
  //! Mirrors register write enable from inside the CPU.
  assign debug_RegWrite = cpu_inst.RegWrite;
  //! Mirrors register writeback source selector from inside the CPU.
  assign debug_RegDest = cpu_inst.RegDest;
  //! Mirrors memory operation mode from inside the CPU.
  assign debug_MemOp = cpu_inst.MemOp;
  //! Mirrors equality branch control from inside the CPU.
  assign debug_BranchEqual = cpu_inst.BranchEqual;
  //! Mirrors less-than branch control from inside the CPU.
  assign debug_BranchLt = cpu_inst.BranchLt;
  //! Mirrors jump decode from inside the CPU.
  assign debug_isJump = cpu_inst.isJump;
  //! Mirrors long jump decode from inside the CPU.
  assign debug_LongJump = cpu_inst.LongJump;
  //! Mirrors immediate-load decode from inside the CPU.
  assign debug_isImmLoad = cpu_inst.isImmLoad;
  //! Mirrors ALU operation selector from inside the CPU.
  assign debug_ALUCtrl = cpu_inst.ALUCtrl;
  //! Mirrors IMR lane selector from inside the CPU.
  assign debug_ImmSel = cpu_inst.ImmSel;
  //! Mirrors packed compare/jump selector from inside the CPU.
  assign debug_CMPType = cpu_inst.CMPType;
  //! Mirrors Ra read selector from inside the CPU.
  assign debug_ReadRegRa = cpu_inst.ReadRegRa;
  //! Mirrors Rb read selector from inside the CPU.
  assign debug_ReadRegRb = cpu_inst.ReadRegRb;
  //! Mirrors write register selector from inside the CPU.
  assign debug_WriteReg = cpu_inst.WriteReg;
  //! Mirrors Ra read data from inside the CPU.
  assign debug_ReadDataRa = cpu_inst.ReadDataRa;
  //! Mirrors Rb read data from inside the CPU.
  assign debug_ReadDataRb = cpu_inst.ReadDataRb;
  //! Mirrors ALU operand A from inside the CPU.
  assign debug_OperandA = cpu_inst.OperandA;
  //! Mirrors ALU operand B from inside the CPU.
  assign debug_OperandB = cpu_inst.OperandB;
  //! Mirrors ALU result from inside the CPU.
  assign debug_ALURes = cpu_inst.ALURes;
  //! Mirrors ALU equality flag from inside the CPU.
  assign debug_zero = cpu_inst.zerof;
  //! Mirrors ALU less-than flag from inside the CPU.
  assign debug_altb = cpu_inst.altb;
  //! Mirrors register writeback data from inside the CPU.
  assign debug_RegDataWrite = cpu_inst.RegDataWrite;
  //! Mirrors final branch decision from inside the CPU.
  assign debug_WillBranch = cpu_inst.WillBranch;
  //! Mirrors next IC candidate from inside the CPU.
